		...
		static std::shared_ptr<Mesh> genSphere(){
			...
			for (int i = 0; i < resolution; i++) {
				...
				for (int j = 0; j < resolution; j++) {
					...
					retVal->m_vertexPositions.push_back(sin(teta) * cos(phi));
					retVal->m_vertexPositions.push_back(sin(teta) * sin(phi));
					retVal->m_vertexPositions.push_back(cos(teta));

					retVal->m_vertexNormals.push_back(sin(teta) * cos(phi));
					retVal->m_vertexNormals.push_back(sin(teta) * sin(phi));
					retVal->m_vertexNormals.push_back(cos(teta));

					retVal->m_vertexTexCoords.push_back(i/resolution);
					retVal->m_vertexTexCoords.push_back(j/resolution);
			...
			/*(i,j) dans le contexte =
			* indice 16*i + j
			* donc (i,j) (i,j+1) (i+1,j+1)
			* 16*i + j -> 16*i + j + 1 -> 16*(i+1) + j + 1
			*/
			return retVal;
...
void render() {
...

  {
	  ...
	  glUniformMatrix4fv(glGetUniformLocation(g_program, "modelMat"), 1, GL_FALSE, glm::value_ptr(modelMatrix));
	  glUniform3f(glGetUniformLocation(g_program, "sphereColor"), sphereColor[0], sphereColor[1], sphereColor[2]);
	  glUniform1i(glGetUniformLocation(g_program, "isSun"), 2);
  }
  g_sphere->render();
}
